  ...
  return (
    background.map((item: { type: string; template_name: any, url: string }) => {
      if (item.type === "template" && item.template_name) {
        ...
        return (
          <div className="bg-cover bg-no-repeat w-full h-screen flex text-white text-4xl uppercase" style={{ backgroundImage: `url("${item.url}")` }}>
            {
              <div className="grid grid-cols-2 gap-4 w-full overflow-y-hidden overflow-hidden">
